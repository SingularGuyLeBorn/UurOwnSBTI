 ...
 */
const CONNECTORS = {
  opening: ['绝了', '卧槽', '他妈的', '神了', '服了', '离谱', '离谱他妈给离谱开门', '好家伙', '好家伙我直接好家伙'],
  transition: ['您这人', '说实话', '讲真', '不瞒您说', '坦白讲', '有一说一', '说句公道话', '不是我说您'],
  warning: ['危险预警', '问题在于', '可怕的是', '最绝的是', '离谱的是', '更离谱的是', '最离谱的是'],
  advice: ['离谱建议', '个人建议', '说真的', '建议您', '给您个建议', '肺腑之言', '掏心窝子说']
};
...
const DEEP_ANALYSIS_TEMPLATES = [
  '从您的答题模式来看，您是一个典型的{type}。您的思维方式就像{type}的关键词描述的那样：{keywords}。这种性格特质在当代社会既是诅咒也是祝福——诅咒是您永远和"正常人"格格不入，祝福是您至少知道自己不正常在哪里。',
  '深入分析您的选择，我们发现您的{type}属性高达{score}分。这意味着您比{percent}%的人更像{type}。您的关键词{keywords}不是标签，是您的生存状态。您活成了{type}的教科书案例，可惜这本教科书没人想看。',
  '您的测试结果显示，您的{type}特质非常突出。{keywords}这些词就像是为您的灵魂量身定制的。您可能一直在寻找同类，但{type}的悲哀在于——同类相斥，异类相吸，您注定孤独。',
  '从心理学角度看，您的{type}人格形成于长期的逃避机制。{keywords}这些关键词揭示了您内心深处的恐惧和渴望。您害怕被忽视，渴望被理解，但您用{type}的方式把自己包装得严严实实。'
];
...
const LIFE_SCENARIOS = [
  '在工作中，您是那种让人又爱又恨的同事。您的{type}属性让您在会议上表现得格外突出。',
  '在感情中，您的{type}特质让您成为让人窒息的存在。您的伴侣（如果有的话）可能经常怀疑："这人是{type}成精了吗？"答案是：是的。',
  '在社交场合，您的{type}人格会让您在聚会开始10分钟后触发{type}模式。然后您会突然消失。',
  '在深夜独处时，您的{type}属性会达到峰值。您会疯狂刷手机。这就是{type}的宿命：白天装正常人，晚上做自己。'
];
...
const FUTURE_PREDICTIONS = [
  '根据您的{type}属性预测，您未来可能会因为一条朋友圈失眠整晚。但别担心，这是{type}的正常操作。',
  '以您目前的{type}发展趋势，五年后您会成为您最讨厌的那种人。这就是{type}的成长轨迹：螺旋式下降。',
  '您的{type}人格决定了您的人生剧本：高开低走。但好消息是，{type}的低谷没有下限，所以您永远不会失望。',
  '基于您的{type}特质，您的中年危机可能会提前到明天。届时您会疯狂购物，然后发现{type}的本质不变。'
];
...
export function generateRoast(
  ...
  if (hasContradiction) {
    ...
    roast += `您这人格跟薛定谔的猫似的，观测前同时是42种贵物，观测后坍缩成"随便"。\n\n`;
...
export function generateRushiRoast(answered: number, total: number): string {
  ...
  // 深度分析
  roast += `从心理学角度分析，您是${randomPick(metaphors)}。这种性格特质源于您大脑中的多巴胺受体过度敏感，导致您无法延迟满足。您的大脑就像一台超频的CPU，永远在寻找下一个刺激点，永远无法在当下停留。\n\n`;
  ...
  // 危险预警
  roast += `${randomPick(insults)}。更严重的是，您的急躁正在毁掉您的人际关系——朋友发的长语音您从来不听直接转文字，伴侣发的长篇大论您只看最后一句，家人的关心您觉得是啰嗦。您正在用您的快节奏把所有人推远。\n\n`;
  
  // 未来预测
  roast += `以您目前的急躁程度，五年后您可能会因为等不及电梯而爬20层楼，因为等不及红灯而闯红灯被撞，因为等不及爱情而随便找个人结婚然后离婚。您的墓志铭可能会是："这里躺着一个人，他等不及死了。"\n\n`;
  
  // 建议
  roast += `建议您去送外卖，但估计您也等不及商家出餐。适合去炒股，但肯定拿不住股票，涨了就想卖跌了就想割肉。适合去...算了您肯定也等不及我给您建议。您这性格唯一适合的工作是：当一只蜂鸟，因为蜂鸟的心跳每分钟1200次，和您一样永远停不下来。\n\n`;
...
export function generateRandRoast(pseudoType?: SBTITypeCode): string {
  ...
  // 深度分析
  roast += `从存在主义角度看，您的行为是一种极端的"选择逃避"。您害怕为自己的选择负责，所以把决定权交给随机。这背后是一种深层的不安全感——您不相信自己的判断，或者说，您根本没有形成稳定的自我。您就像一个空壳，里面装满了${currency}抛出来的随机结果。\n\n`;
  ...
  // 建议
  roast += `建议您去买彩票，反正您也懒得兑奖。或者去当随机数生成器测试员，专门测试各种随机算法的随机性。或者去当占卜师，反正都是瞎猜，您的瞎猜至少还有${currency}背书。但说真的，建议您还是试着自己做一次决定——哪怕只是决定今晚吃什么。 small steps, you know？\n\n`;
  
  // 总结
  roast += `最后想说，随机不是自由，逃避不是解脱。您用${currency}逃避选择，但逃避本身也是一种选择——选择不选择。这个悖论可能会让您的大脑死机，但没关系，死机了就用${currency}决定要不要重启。祝您在随机的人生里，偶尔也能 deterministic 一次。`;
...
function generateRushiResultRoast(result: TestResult): string {
  const answered = result.allScores ? Object.values(result.allScores).filter(s => s > 0).length : 5;
  const total = result.allScores ? Object.keys(result.allScores).length : 15;
